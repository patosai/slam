...
selected_matches = [match for match in matches if match.distance < 48]
...
if False:
    ...
    line_equations = [fundamental_matrix @ np.asarray([img1_kp[match.trainIdx].pt[1], img1_kp[match.trainIdx].pt[0], 1])
    ...
    right_img = img2.copy()
    for match in matches_to_draw:
        cv2.circle(right_img,
                   (int(img2_kp[match.queryIdx].pt[0]), int(img2_kp[match.queryIdx].pt[1])),
                   15,
                   (255, 0, 0),
                   thickness=4)
...
rotation, translation = essential.essential_matrix_to_rotation_translation(essential_matrix)
print(rotation)
print(translation)
